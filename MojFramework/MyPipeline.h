...
public:
	...
	public:
		Vertex() = default;
		Vertex(const Vec3& pos, const Color& c = Colors::White)
			:
			pos(pos),
			color(c)
		{
		...
		{
			pos += rhs.pos; 
			//color = Color::Lerp(color, rhs.color, 0.5f);
			return *this;
		}
		...
		{
			pos *= rhs;
			//color = Color::Lerp(Colors::Black, color, rhs);
			return *this;
		}
...
private:
	...
	void ProcessVertices(const std::vector<Vertex>& vertices, const std::vector<size_t>& indices)
	{
		...
		{
			verticesOut.emplace_back(v.pos * rotation + translation, v.color);
		}
		...
		Vertex itEdge1)
	{
		...
		for (int y = yStart; y < yEnd; y++, itEdge0 += dv0, itEdge1 += dv1)
		{
			...
			// Delta along scanline
			float dx = itEdge1.pos.x - itEdge0.pos.x;

			// iLine starts at left edge
			Vertex iLine = itEdge0;

			// Delta per pixel along scanline (position + color)
			Vertex dScan = (dx == 0.0f) ? Vertex{} : (itEdge1 - itEdge0) / dx;

			// Prestep to xStart
			iLine += dScan * (float(xStart) + 0.5f - itEdge0.pos.x);

			for (int x = xStart; x < xEnd; x++, iLine += dScan)
			{
				// PutPixel with interpolated color
				gfx.PutPixel(x, y, iLine.color);
			}
		}
	}
private:
